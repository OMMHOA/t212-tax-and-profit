...
class ProfitLossProcessor(EventProcessor):
    ...
    def process_buy(self, event: Event):
        ...
        buy_amount = event.total_eur - event.french_tax - event.conversion_fee_eur - event.stamp_duty_tax_eur
        buys = self._buys(event.isin)
        while buys.next is not None:
            buys = buys.next
        buys.next = self.Buy(ts=event.time, amount=buy_amount, shares=event.shares_count)


    def process_sell(self, event: Event):
        self._register_stock(event)
        profit = event.total_eur - event.french_tax - event.conversion_fee_eur - event.stamp_duty_tax_eur
    ...
    def result(self) -> list[ProfitLoss]:
        def get_profitloss(isin, stock) -> self.ProfitLoss:
            ...
            return self.ProfitLoss(stock=stock, profitloss=profit, remaining_shares=shares, remaining_invested_amount=invested)
    ...
    def _remaining_shares(self, isin: str) -> Tuple[Decimal, Decimal]:
        def _sum(buy: self.Buy, shares: Decimal, invested: Decimal) -> Tuple[Decimal, Decimal]:
            if buy is None:
    ...
    def _register_stock(self, event: Event) -> None:
        if event.isin not in self.__stocks:
            self.__stocks[event.isin] = Stock(ticker=event.ticker, name=event.name, isin=event.isin)
